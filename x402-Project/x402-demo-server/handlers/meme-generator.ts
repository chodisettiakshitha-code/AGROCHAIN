...
// Optional canvas import with fallback for non-native environments
...
try {
  ...
  // Mock canvas functions if native canvas module is not installed
  createCanvas = () => ({
    getContext: () => ({ fillText: () => {}, drawImage: () => {}, measureText: () => ({ width: 100 }) }),
